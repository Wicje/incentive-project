...
export default function Sidebar({ className, isMobileView, onClose }: { className?: string, isMobileView?: boolean, onClose?: () => void }) {
  ...
  return (
    <aside className={cn(className, "relative")}>
      ...
      {mounted && identity && (
        <div className="p-3 mt-auto border-t border-[#EFEFEF]">
          <div className="flex items-center justify-between px-2 mb-2">
            <span className="text-xs font-semibold text-stone-500">Theme</span>
            <button
              onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}
              className="p-1.5 rounded-md hover:bg-black/5 dark:hover:bg-white/10 transition-colors text-stone-500"
            >
              {theme === 'dark' ? <Sun className="w-3.5 h-3.5" /> : <Moon className="w-3.5 h-3.5" />}
            </button>
          </div>
          <div className="flex items-center gap-2 p-2 rounded-md hover:bg-black/5 dark:hover:bg-white/5 transition-colors cursor-pointer">
            <div className="w-6 h-6 rounded bg-stone-200 dark:bg-stone-800 flex items-center justify-center text-xs font-semibold text-stone-800 dark:text-stone-300">
